...
const nfcScan = async (req, res) => {
  ...
  try {
    ...
    if (!nfcDocument) {
      return res
        .status(404)
        .json({ error: "NFC document not found for the given tag ID" });
    }
...
const nfcList = async (req, res) => {
  ...
  try {
    const list = await NfcSession.find({
...
const createNFC = async (req, res) => {
  const { productid, hashnumber } = req.body;
  console.log(productid, hashnumber);
  try{
  const postdata = new Nfc({
    tagId: hashnumber,
    tagType: "QrCode",
    tagCounter: 0,
    product: productid,
  ...
  const savetags= await  postdata.save()
  console.log("save nfc", savetags)
} catch (error) {
...
export { nfcScan, nfcSession, createNFC, nfcList };
